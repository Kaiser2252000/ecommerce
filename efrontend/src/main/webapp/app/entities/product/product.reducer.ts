...
export const getEntitiesNoPageable = createAsyncThunk('product/fetch_entity_list_no_pageable', async() =>{
  const requestUrl = `${apiUrl}/nopageable`;
  return axios.get<IProduct[]>(requestUrl)
})

export const getEntitiesByCategoryId = createAsyncThunk('product/fetch_entity_list', async ({categoryId }: IQueryParams) => {
  const requestUrl = `${apiUrl}/category/${categoryId}`;
  return axios.get<IProduct[]>(requestUrl);
});

export const searchEntities = createAsyncThunk( 'product/search_entities', async({query, categoryId}:IQueryParams) => {

  const requestUrl = `${apiUrl}/search${categoryId?`?query=${query}&categoryId=${categoryId}`:`?query=${query}`}`
  return axios.post<IProduct[]>(requestUrl);
})
...
export const ProductSlice = createEntitySlice({
  ...
  extraReducers(builder) {
    builder
      ...
      .addMatcher(isPending(getEntities, getEntity, searchEntities, getEntitiesNoPageable), state => {
