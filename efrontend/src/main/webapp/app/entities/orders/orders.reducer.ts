...
import {
  ...
  createEntitySlice,
  EntityState,
  serializeAxiosError, IUpdateStatus,
...
export const getEntitiesByUserId = createAsyncThunk('orders/fetch_entity_list', async ({ orderStatus, userId  }: IQueryParams) => {
  const requestUrl = `${apiUrl}/userId${orderStatus?`?userId=${userId}&orderStatus=${orderStatus}`:`?userId=${userId}`}`
  return axios.get<IOrders[]>(requestUrl);
});
...
export const createEntity = createAsyncThunk(
  ...
  async (entity: IOrders, thunkAPI) => {
    const result = await axios.post<IOrders>(apiUrl, entity);
...
export const cancelOrder = createAsyncThunk(
  ...
  async(orderId:number, thunkAPI) =>{
    const requestUrl = `${apiUrl}/cancel/${orderId}`;
