...
export const getUsersAsAdminNoPageable = createAsyncThunk('userManagement/fetch_entity_list_no_pageable', async() =>{
  const requestUrl = `${adminUrl}/nopageable`;
  return axios.get<IUser[]>(requestUrl)
})
...
export const saveImage = createAsyncThunk(
  'userManagement/save_image',
  async (data:FormData, thunkAPI) => {
    const requestUrl = `${adminUrl}/saveImage`;
...
export const UserManagementSlice = createSlice({
  ...
  extraReducers(builder) {
    builder
      ...
      .addMatcher(isFulfilled(getUsersAsAdminNoPageable), (state, action) => {
        state.loading = false;
        state.users = action.payload.data;
      })
      ...
      .addMatcher(isRejected(getUsers, getUsersAsAdmin, getUser, getRoles, createUser, updateUser, deleteUser, getUsersAsAdminNoPageable), (state, action) => {
        state.loading = false;
        state.updating = false;
        state.updateSuccess = false;
