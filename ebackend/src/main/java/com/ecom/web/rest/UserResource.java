...
import com.ecom.web.rest.errors.EmailAlreadyUsedException;
import com.ecom.web.rest.errors.LoginAlreadyUsedException;
...
public class UserResource {
    public static String uploadDir = System.getProperty("user.dir") + "/src/main/webapp/content/userImages";
...
//    ) {
//        log.debug("REST request to search Users for an admin");
//        if (!onlyContainsAllowedProperties(pageable)) {
//            return ResponseEntity.badRequest().build();
//        }
//
//        final Page<AdminUserDTO> page = userService.searchUsers(pageable, search);
//        HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(ServletUriComponentsBuilder.fromCurrentRequest(), page);
//        return new ResponseEntity<>(page.getContent(), headers, HttpStatus.OK);
//    }

    @PostMapping("/users/saveImage")
    @PreAuthorize("hasAuthority(\"" + AuthoritiesConstants.ADMIN + "\")")
    public ResponseEntity<Void> saveImage(@RequestBody MultipartFile file) throws IOException {
    ...
    @GetMapping("/users/nopageable")
    @PreAuthorize("hasAuthority(\"" + AuthoritiesConstants.ADMIN + "\")")
    public ResponseEntity<List<AdminUserDTO>> getAllUsersNoPageable() {
        log.debug("REST request to get all User");
        List<AdminUserDTO> adminUserDTOS = userService.findAllNoPageable();
